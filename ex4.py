...
file1 = """ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
file2 = """ecl:#eef340 eyr:2023 hcl:#c0946f pid:244684338 iyr:2020 cid:57 byr:1969 hgt:152cm
...
newlist = []
for passport in lst:
	dct_passport = passport.replace("\n", " ")
	fields = dct_passport.split(" ")
	updated_fields = ", ".join(fields)
	# print(updated_fields)
	fieldlist = []
	for field in fields:
		fieldlist.append(map(str.strip, field.split(":")))
	newlist.append(dict(fieldlist))


# print(newlist)
# expected_fields = ("ecl", "pid", "eyr", "hcl", "byr", "iyr", "hgt")
# count = 0
# for passport in newlist:
# 	if all (key in passport for key in expected_fields):
# 		count += 1
		...
		if len(passport["byr"])==4 and int(passport["byr"])>=1920 and int(passport["byr"])<=2002:
			if len(passport["iyr"])==4 and int(passport["iyr"])>=2010 and int(passport["iyr"])<=2020:
				if len(passport["eyr"])==4 and int(passport["eyr"])>=2020 and int(passport["eyr"])<=2030:
					if (len(passport["hgt"].split("cm"))==2 and int(passport["hgt"].split("cm")[0])>=150 and int(passport["hgt"].split("cm")[0])<=193) or (len(passport["hgt"].split("in"))==2 and int(passport["hgt"].split("in")[0])>=59 and int(passport["hgt"].split("in")[0])<=76):
						if len(passport["hcl"])==7 and re.search("^\#[a-f0-9]", passport["hcl"]):
							if passport["ecl"] in ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]:
								if len(passport["pid"])==9 and re.search("[0-9]", passport["pid"]):
									count += 1
